...
fn test_ai_suggestion_creation() {
    let suggestion = AiSuggestion {
        line: 42,
        column: 15,
        message: "Consider using optional chaining (?.) instead of manual null check".to_string(),
        severity: SuggestionSeverity::Warning,
        rule_id: Some("prefer-optional-chaining".to_string()),
        suggested_fix: Some("Replace `item && item.property` with `item?.property`".to_string()),
...
fn test_suggestion_categories() {
    ...
    for category in categories {
        ...
        let deserialized: SuggestionCategory = serde_json::from_str(&serialized).expect("Should deserialize");
...
fn test_suggestion_filtering_logic() {
    ...
    let high_confidence: Vec<_> = suggestions.iter().filter(|s| s.confidence >= 0.8).collect();
...
fn test_language_confidence_adjustment() {
    ...
    let ts_multiplier = language_preferences.get("typescript").copied().unwrap_or(1.0);
    ...
    let js_multiplier = language_preferences.get("javascript").copied().unwrap_or(1.0);
    ...
    let unknown_multiplier = language_preferences.get("python").copied().unwrap_or(1.0);
...
fn test_large_batch_processing() {
    ...
    let filtered: Vec<_> = large_suggestions.into_iter().filter(|s| s.confidence >= min_confidence).collect();
    ...
    let high_confidence_count = filtered.iter().filter(|s| s.confidence >= 0.9).count();
...
fn test_oxc_parser_error_conversion() {
    ...
    assert!(
        diag.message.contains("error") || diag.message.contains("invalid"),

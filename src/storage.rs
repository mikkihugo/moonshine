...
impl HybridStorage {
  ...
  async fn try_load_with_assemblage_kv(&mut self) -> Result<()> {
    ...
    let storage_backend =
      storage::open(KV_STORAGE_PATH).await.map_err(|e| {
        Error::config(format!("Failed to open assemblage_kv storage: {:?}", e))
      })?;

    let store = KvStore::open(storage_backend).await.map_err(|e| {
      Error::config(format!("Failed to open KV store: {:?}", e))
    })?;

    // Load prompts from Moon config into assemblage_kv
    if let Ok(Some(prompts_json)) = get_moon_config_safe("moonshine_prompts") {
      if let Ok(prompts_data) = serde_json::from_str::<
        ...
        for (key, value) in prompts_data {
          if let Ok(value_str) = serde_json::to_string(&value) {
            current
              ...
              .map_err(|e| {
                Error::config(format!("Failed to insert prompt: {:?}", e))
              })?;
          }
        }

        current.commit().await.map_err(|e| {
          Error::config(format!("Failed to commit prompts: {:?}", e))
        })?;
      }
    }
    ...
    {
      if let Ok(training_data) = serde_json::from_str::<
        HashMap<String, serde_json::Value>,
      >(&training_json)
      {
        let mut current = store.current().await;
        ...
        current.commit().await.map_err(|e| {
          Error::config(format!("Failed to commit training data: {:?}", e))
        })?;
      }
    }
    ...
    if let Ok(Some(metrics_json)) = get_moon_config_safe("moonshine_metrics") {
      if let Ok(metrics_data) = serde_json::from_str::<
        HashMap<String, serde_json::Value>,
      >(&metrics_json)
      {
        let mut current = store.current().await;
        ...
        for (key, value) in metrics_data {
          if let Ok(value_str) = serde_json::to_string(&value) {
            current
              ...
              .map_err(|e| {
                Error::config(format!("Failed to insert metrics: {:?}", e))
              })?;
          }
        }

        current.commit().await.map_err(|e| {
          Error::config(format!("Failed to commit metrics: {:?}", e))
        })?;
      }
    }
  ...
  fn save_prompts_to_file(&self) -> Result<()> {
    ...
    let json_content =
      serde_json::to_string_pretty(&prompts_map).map_err(|e| {
        Error::config(format!("Failed to serialize prompts: {}", e))
      })?;

    write_file_atomic(
      &format!("{}/prompts.json", self.base_path),
      &json_content,
    )
    .map_err(|e| Error::config(format!("Failed to save prompts: {}", e)))
  }
  ...
  fn save_training_to_file(&self) -> Result<()> {
    ...
    let json_content =
      serde_json::to_string_pretty(&training_map).map_err(|e| {
        Error::config(format!("Failed to serialize training data: {}", e))
      })?;

    write_file_atomic(
      &format!("{}/training.json", self.base_path),
      &json_content,
    )
  ...
  fn save_metrics_to_file(&self) -> Result<()> {
    ...
    let json_content =
      serde_json::to_string_pretty(&metrics_map).map_err(|e| {
        Error::config(format!("Failed to serialize metrics: {}", e))
      })?;

    write_file_atomic(
      &format!("{}/metrics.json", self.base_path),
      &json_content,
    )
    .map_err(|e| Error::config(format!("Failed to save metrics: {}", e)))
  }
}
...
impl HybridStorage {
  ...
  async fn try_get_prompt_from_assemblage_kv(
    &self,
    key: &str,
    ...
    match current.get::<&str, Vec<u8>>(PROMPTS_SLOT, &key).await {
      ...
      Err(e) => Err(Error::config(format!(
        "Failed to get prompt from assemblage_kv: {:?}",
        e
      ))),
    }
  }
  ...
  async fn try_save_prompt_to_assemblage_kv(
    &mut self,
    key: &str,
    prompt: &str,
  ) -> Result<()> {
    ...
    current.commit().await.map_err(|e| {
      Error::config(format!("Failed to commit prompt: {:?}", e))
    })?;
  ...
  pub async fn save_optimized_prompt_with_version(
    &mut self,
    key: &str,
    version: &str,
    prompt: &str,
    ...
    let prompt_json = serde_json::to_string(&prompt_data).map_err(|e| {
      Error::config(format!("Failed to serialize prompt data: {}", e))
    })?;
  ...
  pub async fn save_optimized_prompt(
    &mut self,
    key: &str,
    prompt: &str,
    score: f64,
    ...
    self
      .save_optimized_prompt_with_version(key, "1.0.0", prompt, score)
      .await
  }
  ...
  /// @auto-increment true
  /// @dspy-optimized true
  pub async fn save_dspy_optimized_prompt(
    &mut self,
    key: &str,
    prompt: &str,
    score: f64,
    ...
    self
      .save_optimized_prompt_with_version(key, &new_version, prompt, score)
      .await
  }

  /// AI Semantic Versioning Helper - Auto-increment for user edits
  pub async fn save_user_edited_prompt(
    &mut self,
    key: &str,
    prompt: &str,
    score: f64,
  ...
  pub async fn save_new_prompt_version(
    &mut self,
    key: &str,
    prompt: &str,
    score: f64,
...
impl HybridStorage {
  ...
  /// @batch-friendly true
  pub async fn save_training_example(
    &mut self,
    id: &str,
    example: &TrainingExample,
  ) -> Result<()> {
    let example_json = serde_json::to_string(example).map_err(|e| {
      Error::config(format!("Failed to serialize training example: {}", e))
    })?;

    // Always update cache
    self
      .training_cache
      .insert(id.to_string(), example_json.clone());

    // Try to save to assemblage_kv
    let _ = self
      .try_save_training_to_assemblage_kv(id, &example_json)
      .await;
  ...
  async fn try_save_training_to_assemblage_kv(
    &mut self,
    id: &str,
    example_json: &str,
  ) -> Result<()> {
    ...
    current
      ...
      .map_err(|e| {
        Error::config(format!("Failed to insert training example: {:?}", e))
      })?;

    current.commit().await.map_err(|e| {
      Error::config(format!("Failed to commit training example: {:?}", e))
    })?;
...
impl HybridStorage {
  ...
  pub async fn save_metrics(
    ...
    self
      .metrics_cache
      .insert(key.to_string(), metrics_json.clone());

    // Try to save to assemblage_kv
    let _ = self
      .try_save_metrics_to_assemblage_kv(key, &metrics_json)
      .await;
  ...
  async fn try_save_metrics_to_assemblage_kv(
    &mut self,
    key: &str,
    metrics_json: &str,
  ) -> Result<()> {
...
impl HybridStorage {
  ...
  async fn try_save_session_to_assemblage_kv(
    &mut self,
    key: &str,
    data: &str,
  ) -> Result<()> {
    ...
    current
      ...
      .map_err(|e| {
        Error::config(format!("Failed to insert session data: {:?}", e))
      })?;

    current.commit().await.map_err(|e| {
      Error::config(format!("Failed to commit session data: {:?}", e))
    })?;

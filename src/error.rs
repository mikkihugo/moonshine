...
impl Error {
  ...
  /// @since 1.0.0
  #[must_use]
  pub fn extension_operation_with_context(
    operation: impl Into<String>,
  ...
  pub fn user_message(&self) -> String {
    match self {
      Self::ClaudeCli { message, .. } => {
        format!(
          "AI analysis failed: {}. Falling back to static analysis.",
          message
        )
      }
      Self::AIExecution {
        ...
        format!(
          "AI provider '{}' failed: {}. Falling back to static analysis.",
          provider, message
        )
      }
      Self::Processing { message, .. } => {
        format!(
          "Processing error: {}. Please retry or check logs for details.",
          message
        )
      }
      Self::Storage { message, .. } => {
        format!(
          "Storage error: {}. Please check storage configuration or retry.",
          message
        )
      }
      ...
      Self::Analysis {
        ...
        if let Some(file_path) = file_path {
          format!(
            "Analysis error during '{}' on '{}': Please check file syntax",
            operation, file_path
          )
        } else {
      ...
      Self::Serialization { .. } => {
        "Data processing error: Please check input format".to_string()
      }
      Self::Io { path, .. } => format!(
        "File operation failed on '{}': Please check file permissions",
        path
      ),
      Self::ExtismPdk { .. } => {
        "Extension runtime error: Please restart the operation".to_string()
      }
      Self::Wasm { operation, .. } => {
        format!("Runtime error during '{}': Please try again", operation)
      }
      Self::MoonPdk { function, .. } => format!(
        "Moon operation '{}' failed: Please check Moon configuration",
        function
      ),
      Self::PatternDetection { pattern_type, .. } => format!(
        "Pattern detection failed for '{}': Continuing with basic analysis",
        pattern_type
      ),
      Self::CoproOptimization { stage, .. } => format!(
        "Optimization failed at '{}': Using default configuration",
        stage
      ),
      Self::Timeout { task_name, .. } => format!(
        "Operation '{}' timed out: Please try with simpler input",
        task_name
      ),
      Self::Validation {
        field, expected, ..
      } => format!("Invalid '{}': Expected {}", field, expected),
      Self::Multiple {
        errors,
        successful_count,
      } => {
        format!(
          "Partial completion: {} operations succeeded, {} failed",
          successful_count,
          errors.len()
        )
      }

...
impl CodePatternMatcher {
  ...
  pub fn with_patterns(patterns: ConfigurablePatterns) -> Self {
    Self {
      function_patterns: AhoCorasick::new(&patterns.function_patterns)
        .unwrap_or_else(|_| {
          AhoCorasick::new(Self::get_default_function_patterns()).unwrap()
        }),
      conditional_patterns: AhoCorasick::new(&patterns.conditional_patterns)
        .unwrap_or_else(|_| {
          AhoCorasick::new(Self::get_default_conditional_patterns()).unwrap()
        }),
      loop_patterns: AhoCorasick::new(&patterns.loop_patterns).unwrap_or_else(
        |_| AhoCorasick::new(Self::get_default_loop_patterns()).unwrap(),
      ),
      comment_patterns: AhoCorasick::new(&patterns.comment_patterns)
        .unwrap_or_else(|_| {
          AhoCorasick::new(Self::get_default_comment_patterns()).unwrap()
        }),
      test_patterns: AhoCorasick::new(&patterns.test_patterns).unwrap_or_else(
        |_| AhoCorasick::new(Self::get_default_test_patterns()).unwrap(),
      ),
      security_patterns: AhoCorasick::new(&patterns.security_patterns)
        .unwrap_or_else(|_| {
          AhoCorasick::new(Self::get_default_security_patterns()).unwrap()
        }),
      performance_patterns: AhoCorasick::new(&patterns.performance_patterns)
        .unwrap_or_else(|_| {
          AhoCorasick::new(Self::get_default_performance_patterns()).unwrap()
        }),
      typescript_patterns: AhoCorasick::new(&patterns.typescript_patterns)
        .unwrap_or_else(|_| {
          AhoCorasick::new(Self::get_default_typescript_patterns()).unwrap()
        }),
      documentation_patterns: AhoCorasick::new(
        &patterns.documentation_patterns,
      )
      .unwrap_or_else(|_| {
        AhoCorasick::new(Self::get_default_documentation_patterns()).unwrap()
      }),
    }
  }
  ...
  fn load_patterns_from_external_config(
  ) -> crate::error::Result<ConfigurablePatterns> {
    ...
    {
      if let Ok(patterns) =
        Self::load_patterns_from_file_with_validation(&env_path)
      {
        return Ok(patterns);
      }
    }
    ...
    Err(last_error.unwrap_or_else(|| {
      crate::error::Error::config(
        "No external pattern configuration found".to_string(),
      )
    }))
  }

  /// Load patterns from a specific file with comprehensive validation
  fn load_patterns_from_file_with_validation(
    file_path: &str,
  ) -> crate::error::Result<ConfigurablePatterns> {
  ...
  fn parse_patterns_json(
    ...
    if let Some(version) = json_data.get("version").and_then(|v| v.as_str()) {
      if !version.starts_with("1.") {
        return Err(crate::error::Error::config(format!(
          "Incompatible patterns JSON version: {}",
          version
        )));
      }
    }
    ...
    let extract_patterns =
      |key: &str, default_fn: fn() -> Vec<&'static str>| -> Vec<String> {
        json_data
          .get(key)
          ...
          .unwrap_or_else(|| {
            default_fn().into_iter().map(|s| s.to_string()).collect()
          })
      };

    Ok(ConfigurablePatterns {
      function_patterns: extract_patterns(
        "function_patterns",
        Self::get_default_function_patterns,
      ),
      conditional_patterns: extract_patterns(
        "conditional_patterns",
        Self::get_default_conditional_patterns,
      ),
      loop_patterns: extract_patterns(
        "loop_patterns",
        Self::get_default_loop_patterns,
      ),
      comment_patterns: extract_patterns(
        "comment_patterns",
        Self::get_default_comment_patterns,
      ),
      test_patterns: extract_patterns(
        "test_patterns",
        Self::get_default_test_patterns,
      ),
      security_patterns: extract_patterns(
        "security_patterns",
        Self::get_default_security_patterns,
      ),
      performance_patterns: extract_patterns(
        "performance_patterns",
        Self::get_default_performance_patterns,
      ),
      typescript_patterns: extract_patterns(
        "typescript_patterns",
        Self::get_default_typescript_patterns,
      ),
      documentation_patterns: extract_patterns(
        "documentation_patterns",
        Self::get_default_documentation_patterns,
      ),
    })
  }

  /// Load patterns from Moon configuration
  fn load_patterns_from_moon_config(
  ) -> crate::error::Result<ConfigurablePatterns> {
    use crate::moon_pdk_interface::get_moon_config_safe;
    ...
    if let Ok(Some(patterns_json)) = get_moon_config_safe("moonshine_patterns")
    {
      let patterns_data: serde_json::Value =
        serde_json::from_str(&patterns_json).map_err(|e| {
          crate::error::Error::config(format!(
            "Invalid patterns JSON in Moon config: {}",
            e
          ))
        })?;
    ...
    Err(crate::error::Error::config(
      "No patterns found in Moon configuration".to_string(),
    ))
  }
  ...
  fn get_default_configurable_patterns() -> ConfigurablePatterns {
    ConfigurablePatterns {
      function_patterns: Self::get_default_function_patterns()
        .into_iter()
        .map(|s| s.to_string())
        .collect(),
      conditional_patterns: Self::get_default_conditional_patterns()
        .into_iter()
        .map(|s| s.to_string())
        .collect(),
      loop_patterns: Self::get_default_loop_patterns()
        .into_iter()
        .map(|s| s.to_string())
        .collect(),
      comment_patterns: Self::get_default_comment_patterns()
        .into_iter()
        .map(|s| s.to_string())
        .collect(),
      test_patterns: Self::get_default_test_patterns()
        .into_iter()
        .map(|s| s.to_string())
        .collect(),
      security_patterns: Self::get_default_security_patterns()
        .into_iter()
        .map(|s| s.to_string())
        .collect(),
      performance_patterns: Self::get_default_performance_patterns()
        .into_iter()
        .map(|s| s.to_string())
        .collect(),
      typescript_patterns: Self::get_default_typescript_patterns()
        .into_iter()
        .map(|s| s.to_string())
        .collect(),
      documentation_patterns: Self::get_default_documentation_patterns()
        .into_iter()
        .map(|s| s.to_string())
        .collect(),
    }
  ...
  fn get_default_conditional_patterns() -> Vec<&'static str> {
    vec![
      "if", "switch", "case", "?", ":", "else", "elif", "try", "catch",
      "finally",
    ]
  }
  ...
  fn get_default_loop_patterns() -> Vec<&'static str> {
    vec![
      "for", "while", "do", "forEach", "map", "filter", "reduce", "find",
      "some", "every",
    ]
  }
  ...
  pub fn count_patterns(&self, content: &str) -> CodeMetrics {
    CodeMetrics {
      ...
      performance_concerns: self.performance_patterns.find_iter(content).count()
        as f32,
      typescript_usage: self.typescript_patterns.find_iter(content).count()
        as f32,
      documentation_markers: self
        .documentation_patterns
        .find_iter(content)
        .count() as f32,
    }
  ...
  /// @complexity medium
  /// @since 1.0.0
  pub fn analyze_patterns_with_context(
    &self,
    content: &str,
...
impl AiLinter {
  ...
  fn build_from_config(
    mut config: crate::config::MoonShineConfig,
  ...
  fn sync_runtime_from_config(&mut self) {
    self.max_suggestions = self
      ...
      .unwrap_or(DEFAULT_MAX_SUGGESTIONS);
    self.min_confidence =
      self.config.min_confidence.unwrap_or(DEFAULT_MIN_CONFIDENCE);
    self.enable_auto_fix = self
      .config
      .enable_auto_fix
      .unwrap_or(DEFAULT_ENABLE_AUTO_FIX);
  ...
  /// @complexity high
  /// @since 1.0.0
  pub async fn analyze_code_comprehensive(
    &self,
    file_path: &str,
    content: &str,
    language: &str,
    ...
    info!(
      "Starting comprehensive analysis for {} ({})",
      file_path, language
    );
    ...
    match self
      .run_analysis_phase("compilation_critical", file_path, content, language)
      .await
    {
      Ok(mut suggestions) => {
    ...
    match self
      .run_analysis_phase("type_safety", file_path, content, language)
      .await
    {
      Ok(mut suggestions) => {
    ...
    match self
      .run_analysis_phase("code_quality", file_path, content, language)
      .await
    {
      Ok(mut suggestions) => {
    ...
    let mut filtered_suggestions =
      self.filter_suggestions(collected_suggestions);
    ...
    let quality_score =
      self.calculate_quality_score(&metrics, &filtered_suggestions, content);
  ...
  async fn run_analysis_phase(
    ...
    let prompt = self
      .build_phase_specific_prompt(phase_name, file_path, content, language)?;
    let (context, preferred_providers) =
      self.phase_context(phase_name, language, content);
    ...
    debug!(
      "Phase {} routed to provider {}",
      phase_name, response.provider_used
    );

    Ok(self.parse_ai_response(
      &response.content,
  ...
  /// @complexity medium
  /// @since 1.0.0
  fn build_phase_specific_prompt(
    &self,
    phase_name: &str,
    file_path: &str,
    content: &str,
    ...
    prompt_template.render(&context).map_err(|e| {
      Error::config(format!("Failed to render prompt template: {}", e))
    })
  }
  ...
  fn get_phase_prompt_template(
    ...
    let prompt_content = match phase_name {
      "compilation_critical" => {
        get_prompt("pass_1_compilation_critical", Some(&self.custom_prompts))
      }
      "type_safety" => get_prompt(
        "pass_2_type_safety_implementation",
        Some(&self.custom_prompts),
      ),
      "code_quality" => get_prompt(
  ...
  fn parse_ai_response(
    ...
    debug!(
      "Falling back to text parser for provider {} during {} phase",
      provider, phase_name
    );
  ...
  /// @complexity medium
  /// @since 1.0.0
  fn collect_structured_suggestions(
    &self,
    value: &serde_json::Value,
    phase_name: &str,
    sink: &mut Vec<AiSuggestion>,
  ) {
    match value {
      serde_json::Value::Array(items) => {
        ...
        for item in items {
          if let Some(suggestion) =
            self.parse_suggestion_object(item, phase_name)
          {
      ...
      serde_json::Value::Object(map) => {
        for key in [
          ...
          if let Some(array) = map.get(key).and_then(|v| v.as_array()) {
            for entry in array {
              if let Some(suggestion) =
                self.parse_suggestion_object(entry, phase_name)
              {
                sink.push(suggestion);
              } else {
  ...
  fn parse_suggestion_object(
    ...
    let auto_fixable = obj
      ...
      .or_else(|| {
        obj
          .get("fix")
          .and_then(|fix| fix.get("auto").and_then(|v| v.as_bool()))
      })
      .unwrap_or(false);
  ...
  pub fn apply_auto_fix(
    ...
    if !self.enable_auto_fix
      ...
      || suggestion.suggested_fix.is_none()
    {
    ...
    if let Ok(validated_fix) =
      self.apply_validated_string_fix(source, suggestion)
    {
  ...
  fn apply_ast_based_fix(
    ...
    if !errors.is_empty() {
      return Err(Error::config(
        "Source code has parse errors - skipping AST transformation"
          .to_string(),
      ));
    }
    ...
    let transformed_program =
      match suggestion.rule_id.as_ref().map(|s| s.as_str()) {
        Some(rule) if rule.contains("type") => {
          self.apply_type_safety_transform(&allocator, program, suggestion)?
        }
        Some(rule) if rule.contains("performance") => {
          self.apply_performance_transform(&allocator, program, suggestion)?
        }
        Some(rule) if rule.contains("security") => {
          self.apply_security_transform(&allocator, program, suggestion)?
        }
        Some(rule) if rule.contains("style") => {
          self.apply_style_transform(&allocator, program, suggestion)?
        }
        _ => {
          return Err(Error::config(
            "No AST transformation available for this suggestion type"
              .to_string(),
          ));
        }
      };
  ...
  fn apply_type_safety_transform(
    ...
    Err(Error::config(
      "Type safety AST transformations not yet implemented".to_string(),
    ))
  }
  ...
  fn apply_performance_transform(
    ...
    Err(Error::config(
      "Performance AST transformations not yet implemented".to_string(),
    ))
  }
  ...
  fn apply_security_transform(
    ...
    Err(Error::config(
      "Security AST transformations not yet implemented".to_string(),
    ))
  }
  ...
  fn apply_style_transform(
    ...
    Err(Error::config(
      "Style AST transformations not yet implemented".to_string(),
    ))
  }

  /// Apply enhanced string replacement with syntax validation
  fn apply_validated_string_fix(
    &self,
    source: &str,
    suggestion: &AiSuggestion,
  ) -> Result<String> {
    ...
    if !self.is_valid_fix_syntax(fix, suggestion) {
      return Err(Error::config(
        "Suggested fix appears to have invalid syntax".to_string(),
      ));
    }

    // Apply string replacement with enhanced positioning
    let fixed_source =
      self.apply_precise_string_replacement(source, suggestion, fix)?;

    // Post-validation: Quick syntax check on the result
    if !self.validate_fixed_syntax(&fixed_source) {
      return Err(Error::config(
        "Fixed code has syntax errors - reverting".to_string(),
      ));
    }
  ...
  fn is_valid_fix_syntax(&self, fix: &str, suggestion: &AiSuggestion) -> bool {
    ...
    match suggestion.category {
      SuggestionCategory::TypeSafety => {
        ...
        fix_trimmed.contains(':')
          || fix_trimmed.contains("unknown")
          || fix_trimmed.contains("string")
      }
  ...
  fn apply_precise_string_replacement(
    &self,
    source: &str,
    suggestion: &AiSuggestion,
    fix: &str,
    ...
    let replacement_range =
      self.find_replacement_range(line, column, suggestion)?;
  ...
  fn validate_fixed_syntax(&self, source: &str) -> bool {
    ...
    errors.len() < 5
      && !errors.iter().any(|e| e.to_string().contains("Unexpected"))
  }

  /// Fallback to original string replacement method
  fn apply_fallback_string_fix(
    &self,
    source: &str,
    suggestion: &AiSuggestion,
  ) -> String {
  ...
  /// @complexity medium
  /// @since 1.0.0
  pub fn process_batch_suggestions(
    &self,
    ...
    let pattern = Regex::new(
      r"(?i)^Line\s+(?P<line>\d+):\s*(?P<message>.+?)\s*\((?P<severity>critical|error|warning|info|hint)\)\s*$",
    )
  ...
  /// @complexity low
  /// @since 1.0.0
  pub fn adjust_confidence_for_language(
    &self,
    suggestion: &AiSuggestion,
    language: &str,
  ) -> f32 {
  ...
  /// @complexity low
  /// @since 1.0.0
  pub fn group_suggestions_by_rule(
    &self,
    suggestions: Vec<AiSuggestion>,
  ) -> HashMap<String, Vec<AiSuggestion>> {
  ...
  /// @complexity high
  /// @since 1.0.0
  fn parse_text_response_enhanced(
    &self,
    response: &str,
    phase_name: &str,
  ) -> Vec<AiSuggestion> {
    ...
    if let Some(json_suggestions) =
      self.extract_embedded_json_suggestions(response)
    {
    ...
    suggestions
      .extend(self.parse_structured_response_patterns(response, phase_name));
  ...
  fn extract_embedded_json_suggestions(
    &self,
    response: &str,
    ...
    if let Some(json_block) = self.extract_json_from_code_blocks(response) {
      if let Ok(suggestions) =
        serde_json::from_str::<Vec<AiSuggestion>>(&json_block)
      {
    ...
    if let Some(json_array) = self.extract_json_array_patterns(response) {
      if let Ok(suggestions) =
        serde_json::from_str::<Vec<AiSuggestion>>(&json_array)
      {
        return Some(suggestions);
      }
    }
  ...
  fn extract_json_from_code_blocks(&self, response: &str) -> Option<String> {
    let json_block_regex =
      regex::Regex::new(r"```(?:json)?\s*(\[[\s\S]*?\])\s*```").ok()?;
    json_block_regex
      .captures(response)
  ...
  fn extract_suggestion_objects(
    ...
    let suggestion_regex = regex::Regex::new(
      r"(?i)(?:line\s+(\d+)|(\d+):)[\s:]*(.+?)(?:severity|level)[\s:]*(\w+)",
    )
    .ok()?;
  ...
  fn parse_structured_response_patterns(
    &self,
    response: &str,
    phase_name: &str,
  ) -> Vec<AiSuggestion> {
    ...
    for caps in numbered_regex.captures_iter(response) {
      if let Some(content) = caps.get(2) {
        ...
        if self.is_suggestion_content(content_text) {
          suggestions.push(self.create_suggestion_from_pattern(
            content_text,
            phase_name,
            "numbered_list",
            0.70,
    ...
    for caps in line_regex.captures_iter(response) {
      if let (Some(line_match), Some(content)) = (caps.get(1), caps.get(2)) {
        if let Ok(line_num) = line_match.as_str().parse::<u32>() {
          let content_text = content.as_str().trim();
          let mut suggestion = self.create_suggestion_from_pattern(
            content_text,
            phase_name,
            "line_specific",
            0.80,
  ...
  fn parse_semantic_statements(
    ...
    for sentence in sentences {
      if let Some(suggestion) =
        self.analyze_sentence_semantics(sentence, phase_name)
      {
        suggestions.push(suggestion);
      }
    }
  ...
  fn create_suggestion_from_pattern(
    &self,
    content: &str,
    phase_name: &str,
    pattern_type: &str,
  ...
  fn infer_severity_from_context(&self, text: &str) -> SuggestionSeverity {
    ...
    } else if text_lower.contains("warning")
      || text_lower.contains("issue")
      || text_lower.contains("problem")
    {
      SuggestionSeverity::Warning
    } else if text_lower.contains("hint")
      || text_lower.contains("tip")
  ...
  fn infer_category_from_message(&self, message: &str) -> SuggestionCategory {
    ...
    if msg_lower.contains("type")
      || msg_lower.contains("any")
      || msg_lower.contains("undefined")
    {
      SuggestionCategory::TypeSafety
    } else if msg_lower.contains("performance")
      || msg_lower.contains("slow")
      || msg_lower.contains("optimization")
    {
      SuggestionCategory::Performance
    } else if msg_lower.contains("security")
  ...
  fn post_process_suggestions(
    &self,
    mut suggestions: Vec<AiSuggestion>,
  ) -> Vec<AiSuggestion> {
    // Remove duplicates based on message similarity
    suggestions
      .dedup_by(|a, b| levenshtein::levenshtein(&a.message, &b.message) < 5);
  ...
  fn parse_category(&self, category_str: &str) -> SuggestionCategory {
    match category_str.to_lowercase().as_str() {
      "typesafety" | "type_safety" | "typing" | "bug risk" | "bug-risk" => {
        SuggestionCategory::TypeSafety
      }
      ...
      "maintainability" | "maintainable" | "cleanup" => {
        SuggestionCategory::Maintainability
      }
      "modernization" | "modernize" | "migration" => {
        SuggestionCategory::Modernization
      }
      "bestpractices" | "best_practices" | "best-practices" => {
        SuggestionCategory::BestPractices
      }
  ...
  /// @complexity medium
  /// @since 1.0.0
  pub fn parse_claude_response_enhanced(
    &self,
    response: &str,
    phase_name: &str,
  ) -> Vec<AiSuggestion> {
  ...
  fn calculate_quality_score(
    ...
    if let Ok(moon_metrics) = self.calculate_moon_task_metrics(metrics, None) {
      return self
        .compute_quality_from_moon_metrics(&moon_metrics, suggestions);
    }
  ...
  fn calculate_moon_task_metrics(
    &self,
    metrics: &AnalysisMetrics,
    file_path: Option<&str>,
  ) -> Result<MoonTaskMetrics> {
    ...
    write_file_atomic(config_path, &analysis_config.to_string())
      .map_err(|e| Error::analysis(format!("write_file_atomic failed: {}", e)))?;
  ...
  fn execute_moon_metrics_tasks(
    ...
    for task in &tasks {
      ...
      let output = Command::new("moon")
        .args(&["run", task, "--", "--config", config_path])
        .output();
  ...
  fn parse_moon_task_output(
    &self,
    task: &str,
    output: &str,
    results: &mut MoonTaskMetrics,
  ) -> Result<()> {
    match task {
      "moon-shine:analyze-loc" => {
    ...
    if let Ok(cloc_data) = serde_json::from_str::<serde_json::Value>(output) {
      if let Some(languages) = cloc_data.get("languages") {
        ...
        for (_, lang_data) in languages.as_object()? {
          if let Some(code_lines) =
            lang_data.get("code").and_then(|v| v.as_u64())
          {
  ...
  fn parse_maintainability_output(&self, output: &str) -> Option<f32> {
    ...
    if let Ok(data) = serde_json::from_str::<serde_json::Value>(output) {
      if let Some(maintainability) =
        data.get("maintainability_index").and_then(|v| v.as_f64())
      {
  ...
  fn parse_security_output(&self, output: &str) -> Option<f32> {
    ...
    if let Ok(data) = serde_json::from_str::<serde_json::Value>(output) {
      if let Some(security) =
        data.get("security_score").and_then(|v| v.as_f64())
      {
  ...
  fn calculate_ast_based_metrics(
    &self,
    metrics: &AnalysisMetrics,
    source: &str,
  ) -> Result<AstBasedMetrics> {
    use oxc_allocator::Allocator;
    use oxc_parser::{Parser, ParserReturn};
    use oxc_span::SourceType;
    ...
    if !errors.is_empty() {
      return Err(Error::config(
        "Parse errors prevent AST analysis".to_string(),
      ));
    }
  ...
  fn compute_quality_from_moon_metrics(
    &self,
    moon_metrics: &MoonTaskMetrics,
    suggestions: &[AiSuggestion],
  ) -> f32 {
    ...
    let complexity_penalty =
      (moon_metrics.cyclomatic_complexity - 10.0).max(0.0) * 3.0;
    let maintainability_bonus =
      (moon_metrics.maintainability_index - 50.0).max(0.0) * 0.15;
    let security_bonus = (moon_metrics.security_score - 70.0).max(0.0) * 0.1;

    // Suggestion-based penalties with sophisticated weighting
    let suggestion_penalty =
      self.calculate_weighted_suggestion_penalty(suggestions);

    let score =
      base_score - loc_penalty - complexity_penalty - suggestion_penalty
  ...
  fn compute_quality_from_ast_metrics(
    &self,
    ast_metrics: &AstBasedMetrics,
    suggestions: &[AiSuggestion],
  ) -> f32 {
    let base_score = 100.0;
    ...
    let complexity_penalty =
      (ast_metrics.cyclomatic_complexity - 10.0).max(0.0) * 2.5;
  ...
  fn compute_quality_from_enhanced_heuristics(
    &self,
    metrics: &AnalysisMetrics,
    suggestions: &[AiSuggestion],
  ) -> f32 {
    ...
    let maintainability_bonus =
      (metrics.maintainability_index - 50.0).max(0.0) * 0.12;
  ...
  fn calculate_weighted_suggestion_penalty(
    &self,
    suggestions: &[AiSuggestion],
  ) -> f32 {
    suggestions
  ...
  fn calculate_complexity_score(&self, content: &str) -> f32 {
    ...
    let complexity = (metrics.functions * 2.0
  ...
  fn calculate_maintainability_index(
    ...
    let maintainability_issues = suggestions
      .iter()
      .filter(|s| s.category == SuggestionCategory::Maintainability)
      .count() as f32;
    let security_issues = suggestions
      .iter()
      .filter(|s| s.category == SuggestionCategory::Security)
      .count() as f32;
    ...
    (base_index + comment_bonus + test_bonus - issue_penalty - security_penalty)
      .clamp(0.0, 100.0)
  }
  ...
  fn estimate_test_coverage(&self, content: &str) -> f32 {
    ...
    let coverage_estimate =
      ((metrics.test_keywords + 1.0) / (metrics.functions + 1.0)) * 100.0;
  ...
  fn link_related_suggestions(&self, suggestions: &mut [AiSuggestion]) {
    for i in 0..suggestions.len() {
      ...
      for j in 0..suggestions.len() {
        ...
        let line_diff =
          (suggestions[i].line as i32 - suggestions[j].line as i32).abs();
  ...
  pub async fn analyze_code(
    ...
    let analysis_results = self
      .analyze_code_comprehensive(file_path, content, language)
      .await?;
  ...
  fn calculate_metrics(
    ...
    let error_count = suggestions
      ...
      .filter(|s| {
        matches!(
          s.severity,
          SuggestionSeverity::Critical | SuggestionSeverity::Error
        )
      })
      .count() as u32;
    let warning_count = suggestions
      .iter()
      .filter(|s| matches!(s.severity, SuggestionSeverity::Warning))
      .count() as u32;
    let info_count = suggestions
      ...
      .filter(|s| {
        matches!(
          s.severity,
          SuggestionSeverity::Info | SuggestionSeverity::Hint
        )
      })
      .count() as u32;
    let auto_fixable_count =
      suggestions.iter().filter(|s| s.auto_fixable).count() as u32;
    ...
    AnalysisMetrics {
      ...
      maintainability_index: self
        .calculate_maintainability_index(content, suggestions),
...
mod tests {
  ...
  fn test_ai_suggestion_creation() {
    let suggestion = AiSuggestion {
      ...
      suggested_fix: Some(
        "Add type annotation: let x: number = 5;".to_string(),
      ),
  ...
  fn test_suggestion_category_display() {
    ...
    assert_eq!(
      SuggestionCategory::Documentation.to_string(),
      "Documentation"
    );
    assert_eq!(SuggestionCategory::Style.to_string(), "Style");
    assert_eq!(
      SuggestionCategory::Maintainability.to_string(),
      "Maintainability"
    );
    assert_eq!(
      SuggestionCategory::BestPractices.to_string(),
      "BestPractices"
    );

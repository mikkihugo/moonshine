...
impl SecurityAnalyzer {
    ...
    pub fn analyze_security(&self, program: &Program<'_>, semantic: Option<&SemanticBuilderReturn>) -> SecurityAnalysisResult {
        ...
        let issues: Vec<SecurityIssue> = visitor.vulnerabilities.into_iter().map(|vuln| self.vulnerability_to_issue(vuln)).collect();
    ...
    fn classify_vulnerability(&self, vuln_type: &VulnerabilityType) -> (SecurityCategory, SecuritySeverity) {
        match vuln_type {
            VulnerabilityType::CodeInjection | VulnerabilityType::UnsafeEval => (SecurityCategory::Injection, SecuritySeverity::Critical),
            VulnerabilityType::XSS => (SecurityCategory::CrossSiteScripting, SecuritySeverity::High),
            VulnerabilityType::SqlInjection | VulnerabilityType::CommandInjection => (SecurityCategory::Injection, SecuritySeverity::Critical),

...
pub mod compiled; // Zero-runtime-cost compiled patterns
...
// Re-exports
pub use compiled::{all_patterns_iter, enabled_patterns_iter, get_default_pattern_config, patterns_by_category};
...
impl PatternConfig {
    ...
    pub fn load_from_file(path: &str) -> Result<Self> {
        let content = std::fs::read_to_string(path).map_err(|e| Error::config(format!("Failed to read pattern config: {}", e)))?;

        let config: PatternConfig = serde_json::from_str(&content).map_err(|e| Error::config(format!("Failed to parse pattern config: {}", e)))?;
    ...
    pub fn save_to_file(&self, path: &str) -> Result<()> {
        let content = serde_json::to_string_pretty(self).map_err(|e| Error::config(format!("Failed to serialize pattern config: {}", e)))?;

        std::fs::write(path, content).map_err(|e| Error::config(format!("Failed to write pattern config: {}", e)))?;
    ...
    pub fn from_moon_workspace() -> Result<Self> {
        let moon_config = crate::config::MoonShineConfig::from_moon_workspace().unwrap_or_default();
    ...
    pub fn compile(&self) -> Result<CompiledPatterns> {
        ...
        for rule in &self.security_patterns {
            if rule.enabled {
                let regex = Regex::new(&rule.pattern).map_err(|e| Error::config(format!("Invalid security pattern '{}': {}", rule.id, e)))?;
        ...
        for rule in &self.performance_patterns {
            if rule.enabled {
                let regex = Regex::new(&rule.pattern).map_err(|e| Error::config(format!("Invalid performance pattern '{}': {}", rule.id, e)))?;
        ...
        for rule in &self.typescript_patterns {
            if rule.enabled {
                let regex = Regex::new(&rule.pattern).map_err(|e| Error::config(format!("Invalid TypeScript pattern '{}': {}", rule.id, e)))?;
        ...
        for rule in &self.documentation_patterns {
            if rule.enabled {
                let regex = Regex::new(&rule.pattern).map_err(|e| Error::config(format!("Invalid documentation pattern '{}': {}", rule.id, e)))?;
        ...
        for rule in &self.custom_patterns {
            if rule.enabled {
                let regex = Regex::new(&rule.pattern).map_err(|e| Error::config(format!("Invalid custom pattern '{}': {}", rule.id, e)))?;
    ...
    pub fn get_enabled_counts(&self) -> HashMap<String, usize> {
        ...
        counts.insert("security".to_string(), self.security_patterns.iter().filter(|r| r.enabled).count());
        counts.insert("performance".to_string(), self.performance_patterns.iter().filter(|r| r.enabled).count());
        counts.insert("typescript".to_string(), self.typescript_patterns.iter().filter(|r| r.enabled).count());
        counts.insert("documentation".to_string(), self.documentation_patterns.iter().filter(|r| r.enabled).count());
        counts.insert("custom".to_string(), self.custom_patterns.iter().filter(|r| r.enabled).count());
        counts
    }
}
...
mod tests {
    ...
    fn test_json_serialization() {
        ...
        let deserialized: PatternConfig = serde_json::from_str(&json).expect("Should deserialize");

        assert_eq!(config.security_patterns.len(), deserialized.security_patterns.len());
        assert_eq!(config.performance_patterns.len(), deserialized.performance_patterns.len());
    }
}

...
use crate::moon_pdk_interface::{
  get_moon_config, get_moon_config_safe, write_file_atomic,
};
      ...
      if let Some(optimized_prompts) = prompts.get("optimized_prompts") {
        if let Some(prompt_data) = optimized_prompts.get(prompt_name) {
          if let Some(template) =
            prompt_data.get("template").and_then(|v| v.as_str())
          {
    ...
    if let Some(prompts_json) = get_moon_config("moonshine_prompts") {
      ...
      if let Some(copro_candidates) = prompts.get("copro_candidates") {
        if let Some(candidates) =
          copro_candidates.get("active").and_then(|v| v.as_array())
        {
...
fn load_prompts_from_external_storage() -> Result<Vec<PromptTemplate>> {
  ...
  Err(last_error.unwrap_or_else(|| {
    Error::config("No external prompt storage available".to_string())
  }))
}

/// Load prompts from a specific file with retry logic and validation
fn load_prompts_from_file_with_retry(
  file_path: &str,
  max_retries: u32,
...
fn parse_external_prompts_json(
  ...
  if let Some(version) = json_data.get("version").and_then(|v| v.as_str()) {
    if !is_compatible_version(version) {
      return Err(Error::config(format!(
        "Incompatible prompts JSON version: {}",
        version
      )));
    }
  }
  ...
  if let Some(optimized) = json_data
    .get("optimized_prompts")
    .and_then(|v| v.as_object())
  {
    for (name, data) in optimized {
      if let Some(template) = data.get("template").and_then(|v| v.as_str()) {
        templates
          .push(PromptTemplate::new(format!("{}_optimized", name), template));
      }
    }
  }

  // Priority 2: COPRO candidates (high-scoring only)
  if let Some(copro) = json_data
    .get("copro_candidates")
    .and_then(|v| v.as_object())
  {
      ...
      for candidate in active {
        if let (Some(name), Some(template), Some(score)) = (
          ...
          if score > 0.8 {
            ...
            templates
              .push(PromptTemplate::new(format!("{}_copro", name), template));
          }
        }
      }
    }
  ...
  if let Some(base_prompts) =
    json_data.get("base_prompts").and_then(|v| v.as_object())
  {
    for (name, data) in base_prompts {
      if let Some(template) = data.get("template").and_then(|v| v.as_str()) {
        ...
        if !templates
          .iter()
          .any(|t| t.name == optimized_name || t.name == copro_name)
        {
...
fn load_prompts_from_json() -> Result<Vec<PromptTemplate>> {
  let prompts_json =
    get_moon_config_safe("moonshine_prompts_file").or_else(|_| {
  ...
  if let Some(json_content) = prompts_json {
    let prompts_data: serde_json::Value =
      serde_json::from_str(&json_content)
        .map_err(|e| Error::config(format!("Invalid prompts JSON: {}", e)))?;

    let mut templates = Vec::new();

    // Load optimized prompts first (highest priority)
    if let Some(optimized) = prompts_data
      .get("optimized_prompts")
      .and_then(|v| v.as_object())
    {
      for (name, data) in optimized {
    ...
    if let Some(base_prompts) =
      prompts_data.get("base_prompts").and_then(|v| v.as_object())
    {
      for (name, data) in base_prompts {
        if let Some(template) = data.get("template").and_then(|v| v.as_str()) {
    ...
    if let Some(copro) = prompts_data
      .get("copro_candidates")
      .and_then(|v| v.as_object())
    {
      if let Some(active) = copro.get("active").and_then(|v| v.as_array()) {
...
pub fn get_available_rules(
  custom_prompts: Option<&HashMap<String, String>>,
) -> Vec<String> {
...
fn load_rules_from_external_config() -> Result<Vec<String>> {
  ...
  Err(last_error.unwrap_or_else(|| {
    Error::config("No external rules configuration found".to_string())
  }))
}

/// Load rules from a specific file with comprehensive validation
fn load_rules_from_file_with_validation(
  file_path: &str,
) -> Result<Vec<String>> {
  let content = std::fs::read_to_string(file_path).map_err(|e| {
    Error::config(format!("Failed to read rules file {}: {}", file_path, e))
  })?;
  ...
  let json_data: serde_json::Value =
    serde_json::from_str(&content).map_err(|e| {
      Error::config(format!("Invalid JSON in rules file {}: {}", file_path, e))
    })?;
...
fn parse_rules_json(json_data: &serde_json::Value) -> Result<Vec<String>> {
  ...
  if let Some(version) = json_data.get("version").and_then(|v| v.as_str()) {
    if !is_compatible_version(version) {
      return Err(Error::config(format!(
        "Incompatible rules JSON version: {}",
        version
      )));
    }
  }
  ...
  if let Some(active_rules) =
    json_data.get("active_rules").and_then(|v| v.as_array())
  {
  ...
  if rules.is_empty() {
    if let Some(available_rules) =
      json_data.get("available_rules").and_then(|v| v.as_array())
    {
...
fn load_rules_from_moon_config() -> Result<Vec<String>> {
  ...
  {
    let prompts_data: serde_json::Value = serde_json::from_str(&prompts_json)
      .map_err(|e| {
      Error::config(format!("Invalid prompts JSON in Moon config: {}", e))
    })?;
...
pub fn save_optimized_prompt(
  ...
  // Save back to JSON
  let json_content =
    serde_json::to_string_pretty(&prompts_obj).map_err(|e| {
      Error::config(format!("Failed to serialize prompts JSON: {}", e))
    })?;

  write_file_atomic(".moon/moonshine/prompts.json", &json_content).map_err(
    |e| Error::config(format!("Failed to save prompts JSON: {}", e)),
  )?;
...
pub fn save_copro_candidate(
  ...
  if let Some(copro_section) = prompts_obj.get_mut("copro_candidates") {
    if let Some(active_array) = copro_section
      .get_mut("active")
...
fn load_or_create_prompts_json() -> Result<serde_json::Value> {
  if let Ok(Some(json_content)) = get_moon_config_safe("moonshine_prompts_file")
  {
    serde_json::from_str(&json_content).map_err(|e| {
      Error::config(format!("Invalid existing prompts JSON: {}", e))
    })
  } else {
...
pub fn initialize_prompts_storage() -> Result<()> {
  ...
  let json_content =
    serde_json::to_string_pretty(&default_prompts).map_err(|e| {
      Error::config(format!("Failed to serialize default prompts: {}", e))
    })?;

  write_file_atomic(".moon/moonshine/prompts.json", &json_content).map_err(
    |e| Error::config(format!("Failed to initialize prompts storage: {}", e)),
  )?;
...
mod tests {
  ...
  fn test_prompt_rendering() {
    let template =
      PromptTemplate::new("greeting", "Hello {name}, you are {age} years old!");
  ...
  fn test_missing_variable_error() {
    ...
    assert!(result
      .unwrap_err()
      .to_string()
      .contains("Missing variable: name"));
  }

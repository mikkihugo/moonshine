...
use crate::installation::{
  check_moonshine_installed, install_moonshine_extension,
  load_prompt_from_storage,
};
use crate::prompts;
...
/// @complexity high
/// @since 1.0.0
pub fn execute_extension_logic(
  ...
  let operation_mode = args
    .mode
    .as_deref()
    .or(if args.reporting_only {
      Some("reporting-only")
    } else {
      None
    })
    .or(if args.lint_only {
      Some("lint-only")
    } else {
      None
    })
      ...
      match install_moonshine_extension() {
        ...
        Err(e) => {
          ...
          return Err(WithReturnCode::new(
            create_extension_error("Failed to prepare extension installation"),
            1,
          ));
        }
      }
  ...
  info!(
    "Available rules: {} (including custom overrides)",
    available_rules.len()
  );
  ...
  if available_rules.is_empty() {
    ...
    return Err(WithReturnCode::new(
      create_extension_error("No linting rules loaded"),
      1,
    ));
  }
  ...
  let test_prompt =
    load_prompt_from_storage("typescript_strict", Some(&config));
  if test_prompt.is_empty() {
    error!("Prompt system malfunction - no test prompt loaded");
    return Err(WithReturnCode::new(
      create_extension_error("Prompt system not working"),
      1,
    ));
  }
  ...
  let copro_enabled = opt_config
    .get("copro")
    .and_then(|c| c.get("enabled"))
    .and_then(|e| e.as_bool())
    .unwrap_or(true)
    && config.enable_copro_optimization.unwrap_or(true);
  ...
  info!(
    "Workflow request prepared for {} files",
    file_arguments.len()
  );
...
mod tests {
  ...
  fn test_execute_extension_input_serialization() {
    ...
    let deserialized: ExecuteExtensionInput =
  ...
  fn test_get_available_rules_no_config() {
    ...
    assert!(rules
      .iter()
      .any(|rule| rule.contains("typescript") || rule.contains("lint")));
  }
  ...
  fn test_json_serialization_roundtrip() {
    ...
    // Deserialize back
    let restored: ExecuteExtensionInput =

...
impl AstAutoFixEngine {
                ...
                Ok(entry) => {
                    ...
                    if path.is_file() {
                        ...
                        if matches_pattern {
                            ...
                            match self.ignore_matcher.matched(std::path::Path::new(&relative_path.to_string()), path.is_dir()) {
    ...
    pub fn fix_code_ast(&self, code: &str, file_path: &str) -> Result<AstAutoFixResult> {
        ...
        let ret = Parser::new(&allocator, code, source_type).with_options(parser_options).parse();

        let parse_time = parse_start.elapsed().as_millis() as u64;

        if !ret.errors.is_empty() {
            return Err(Error::config(format!("Parse errors in {}: {:?}", file_path, ret.errors)));
        }
        ...
        let semantic_ret = if self.config.enable_semantic_analysis {
            Some(SemanticBuilder::new().with_check_syntax_error(true).build(&program))
        } else {
            None
        };
        ...
        let fixes_applied = self.apply_ast_transformations(&mut program, &allocator, semantic_ret.as_ref(), code, file_path)?;
        ...
        let source_map = sourcemap_builder.map(|builder| builder.into_sourcemap().to_json_string().ok()).flatten();
    ...
    fn fix_type_annotations(&self, program: &Program<'_>, semantic: Option<&SemanticBuilderReturn>) -> Result<Vec<AstFix>> {
        let mut fixes = Vec::new();
        ...
        if fixes.is_empty() {
            fixes.push(AstFix {
                ...
                description: "No 'any' types found - code already has good type safety".to_string(),
                start_line: 1,
                start_column: 1,
                end_line: 1,
                end_column: 1,
    ...
    fn apply_performance_optimizations(&self, program: &Program<'_>, semantic: Option<&SemanticBuilderReturn>) -> Result<Vec<AstFix>> {
        let mut fixes = Vec::new();
        ...
        if let Some(semantic) = semantic {
            ...
            let loop_fixes = self.find_inefficient_loops_semantic(program, semantic)?;
        ...
        if fixes.is_empty() {
            fixes.push(AstFix {
                ...
                description: "No inefficient loops found - performance already optimized".to_string(),
                start_line: 1,
                start_column: 1,
                end_line: 1,
                end_column: 1,
    ...
    fn apply_security_fixes(&self, program: &Program<'_>, semantic: Option<&SemanticBuilderReturn>) -> Result<Vec<AstFix>> {
        let mut fixes = Vec::new();
        ...
        if let Some(semantic) = semantic {
            ...
            let security_fixes = self.find_security_vulnerabilities_semantic(program, semantic)?;
    ...
    fn modernize_syntax(&self, program: &Program<'_>, semantic: Option<&SemanticBuilderReturn>) -> Result<Vec<AstFix>> {
        let mut fixes = Vec::new();
    ...
    fn load_config_from_moon() -> Result<AstAutoFixConfig> {
        ...
        match get_moon_config_safe("moonshine_ast_config") {
            Ok(Some(config_json)) => serde_json::from_str(&config_json).map_err(|e| Error::config(format!("Invalid AST config: {}", e))),
    ...
    pub fn save_results(&self, results: &AstAutoFixResult) -> Result<()> {
        let json_content = serde_json::to_string_pretty(results).map_err(|e| Error::config(format!("Failed to serialize AST results: {}", e)))?;

        let file_path = format!(".moon/moonshine/ast_results_{}.json", results.file_path.replace('/', "_").replace('\\', "_"));

        write_file_atomic(&file_path, &json_content).map_err(|e| Error::config(format!("Failed to save AST results: {}", e)))
    }
    ...
    pub fn format_code(&self, code: &str, file_path: &str) -> Result<String> {
        ...
        let ret = Parser::new(&allocator, code, source_type).with_options(parse_options).parse();

        if !ret.errors.is_empty() {
            return Err(Error::config(format!("Parse errors in {}: {} errors", file_path, ret.errors.len())));
        }
    ...
    fn merge_codegen_options(&self, mut base_options: CodegenOptions) -> CodegenOptions {
        let format_config = &self.config.format_config;

        // Apply formatting preferences over base options
        base_options.single_quote = matches!(format_config.quote_style, QuoteStyle::Single);
    ...
    pub fn fix_and_format(&self, code: &str, file_path: &str) -> Result<AstAutoFixResult> {
    ...
    fn extract_span_from_semantic_error(error: &oxc_diagnostics::Error, source_text: &str) -> (u32, u32) {
        ...
        if let Some(pos) = Self::parse_position_from_error_message(&error_msg, source_text) {
            return pos;
        }
    ...
    fn parse_position_from_error_message(error_msg: &str, source_text: &str) -> Option<(u32, u32)> {
    ...
    fn line_col_to_offset(line: usize, col: usize, source_text: &str) -> Option<(u32, u32)> {
    ...
    fn find_any_type_annotations(&self, program: &Program<'_>, semantic: &SemanticBuilderReturn) -> Result<Vec<AstFix>> {
        let mut fixes = Vec::new();
    ...
    fn find_any_types_regex(&self, program: &Program<'_>) -> Result<Vec<AstFix>> {
        ...
        for (line_num, line) in source_text.lines().enumerate() {
            for m in any_pattern.find_iter(line) {
                fixes.push(AstFix {
                    fix_type: AstFixType::ReplaceAnyType,
                    description: "Replace 'any' type with more specific type annotation".to_string(),
    ...
    fn find_inefficient_loops_semantic(&self, program: &Program<'_>, semantic: &SemanticBuilderReturn) -> Result<Vec<AstFix>> {
        let mut fixes = Vec::new();
    ...
    fn find_inefficient_loops_ast(&self, program: &Program<'_>) -> Result<Vec<AstFix>> {
        ...
        for (line_num, line) in source_text.lines().enumerate() {
            if let Some(m) = inefficient_for_pattern.find(line) {
                fixes.push(AstFix {
                    fix_type: AstFixType::CacheArrayLength,
                    description: "Cache array length in variable to avoid repeated property access".to_string(),
                    start_line: (line_num + 1) as u32,
                    start_column: (m.start() + 1) as u32,
                    end_line: (line_num + 1) as u32,
                    end_column: (m.end() + 1) as u32,
    ...
    fn find_security_vulnerabilities_semantic(&self, program: &Program<'_>, semantic: &SemanticBuilderReturn) -> Result<Vec<AstFix>> {
        let mut fixes = Vec::new();
    ...
    fn find_security_vulnerabilities_ast(&self, program: &Program<'_>) -> Result<Vec<AstFix>> {
...
impl LoopOptimizationVisitor {
    fn new() -> Self {
        Self { inefficient_loops: Vec::new() }
    }

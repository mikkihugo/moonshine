...
pub struct AnalysisResults {
  pub suggestions: Vec<crate::wasm_safe_linter::LintIssue>,
...
fn detect_language_with_fallback(
  file_path: &str,
...
pub fn ai_lint_file_with_content(
  ...
  let analysis_results =
    analyze_file_with_config(&content, &language, &config)?;
  ...
  let training_updates =
    generate_training_updates_from_analysis(&analysis_results)?;
...
fn execute_moon_tasks(
  ...
  let mut moon_response: MoonTaskResponse =
...
fn aggregate_analysis_results(
  ...
  let pattern_insights =
    generate_pattern_insights(quality_score, tsdoc_coverage, &results);
  let (patterns_learned, copro_optimizations) = calculate_learning_counts(
    &training_updates,
    &prompts_updates,
    &semantic_warnings,
  );
  let total_issues = suggestions.len() + semantic_warnings.len();
  let overall_success = determine_overall_success(
    task_success,
    fix_outcome.issues_fixed,
    suggestions.len(),
  );
  let message =
    compose_analysis_message(total_issues, fix_outcome.issues_fixed);
...
mod tests {
  ...
  fn test_resolve_task_name_with_mapping() {
    let mut config = MoonShineConfig::default();
    config.moon_task_mapping = Some(HashMap::from([(
      "rust".to_string(),
      "moon-shine:custom-rust".to_string(),
    )]));
  ...
  fn test_lint_issue_creation() {
    let suggestion = crate::wasm_safe_linter::LintIssue {
      rule_name: "no-unused-vars".to_string(),
      message: "Variable 'x' is never used".to_string(),
      line: 10,
      column: 5,
      severity: crate::wasm_safe_linter::LintSeverity::Warning,
      fix_available: true,
    };
    ...
    assert!(suggestion.fix_available);
  }
  ...
  fn test_analyze_file_with_config() {
    ...
    let result =
      analyze_file_with_config("const x = 1;", "typescript", &config);
    assert!(result.is_ok());
  ...
  fn test_eslint_result() {
    let eslint_result = EslintResult {
      success: true,
      issues: vec![
        serde_json::json!({"ruleId": "no-unused-vars", "severity": "warning"}),
      ],
      fixes_applied: 5,
    };

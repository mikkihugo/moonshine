...
impl ClaudeCLIAdapter {
  ...
  fn load_claude_model_from_config() -> anyhow::Result<String> {
    ...
    if Self::validate_claude_model(default_model)? {
      ...
      Err(anyhow::anyhow!(
        "No valid Claude model available in configuration or defaults"
      ))
    }
  }
  ...
  pub fn generate_provider_optimized_prompt(
    &self,
    base_prompt: &str,
    context: &CodeFixingContext,
  ) -> String {
    let provider_config = self.provider_config.as_ref()
      .unwrap_or(&self.get_default_provider_config());
...
**Return only the improved code with fixes applied.**"#,
  ...
  pub async fn execute_provider_optimized_prompt(
    &self,
    base_prompt: &str,
    context: &CodeFixingContext,
  ...
  async fn execute_moon_task_claude(
    &self,
    ...
    let task_result =
      self.execute_via_moon_host(&task_command, request).await?;

    Ok(task_result)
  }

  /// Execute via Moon host environment using proper PDK patterns
  async fn execute_via_moon_host(
    &self,
    _task_command: &str,
    request: &MoonTaskRequest,
  ) -> Result<String, Box<dyn std::error::Error>> {
  ...
  async fn execute_claude_cli_response(
    &self,
    request: &MoonTaskRequest,
  ) -> Result<String, Box<dyn std::error::Error>> {
    ...
    let optimized_prompt = if prompt.len() > 16384 {
      ...
      if total_lines > 100 {
        ...
        optimized
          .push_str("--- PROMPT START (truncated for optimization) ---\n");
    ...
    let claude_response_json: serde_json::Value =
      serde_json::from_str(&output.stdout)?;

    // Return the fixed_code or improvements as a string
    if let Some(fixed_code) = claude_response_json
      .get("fixed_code")
      .and_then(|v| v.as_str())
    {
      Ok(fixed_code.to_string())
    } else if let Some(improvements) = claude_response_json
      .get("improvements")
  ...
  pub async fn evaluate_prompt_quality(
    &self,
    prompt: &str,
    context: &CodeFixingContext,
    expected_output: &str,
    ...
    let similarity =
      Self::calculate_semantic_similarity(&response, expected_output)?;
    Ok(similarity)
  }
  ...
  fn calculate_semantic_similarity(
    ...
    let edit_distance =
      Self::levenshtein_distance(&actual_norm, &expected_norm);
    let max_len = actual_norm.len().max(expected_norm.len()) as f32;
    let edit_similarity = if max_len > 0.0 {
    ...
    let actual_words: std::collections::HashSet<&str> =
      actual_norm.split_whitespace().collect();
    let expected_words: std::collections::HashSet<&str> =
      expected_norm.split_whitespace().collect();

    let intersection =
      actual_words.intersection(&expected_words).count() as f32;
    let union = actual_words.union(&expected_words).count() as f32;
    let word_similarity = if union > 0.0 {
      intersection / union
    } else {
      0.0
    };
    ...
    let trigram_intersection =
      actual_trigrams.intersection(&expected_trigrams).count() as f32;
    let trigram_union =
      actual_trigrams.union(&expected_trigrams).count() as f32;
    let trigram_similarity = if trigram_union > 0.0 {
      trigram_intersection / trigram_union
    } else {
      0.0
    };

    // 5. Calculate substring containment
    let containment_similarity = if actual_norm.len() >= expected_norm.len() / 2
      && actual_norm.contains(&expected_norm)
    {
      0.8
    } else if expected_norm.len() >= actual_norm.len() / 2
    ...
    let combined_similarity = (edit_similarity * 0.3
      + word_similarity * 0.3
...
impl MoonShineDSPy {
  pub fn new(session_id: String, model: String) -> Self {
    ...
    let claude_adapter = ClaudeCLIAdapter::with_provider_detection(session_id.clone(), model.clone())
      .unwrap_or_else(|_| ClaudeCLIAdapter::new(session_id, model));
  ...
  pub fn get_base_prompts() -> BasePromptCollection {
    Self::load_base_prompts_from_config()
      .unwrap_or_else(|_| Self::get_default_base_prompts())
  }
  ...
  fn load_base_prompts_from_config() -> anyhow::Result<BasePromptCollection> {
    ...
    if let Ok(Some(eslint_prompt)) =
      get_moon_config_safe("extension.moonshine.prompts.eslint_fix")
    {
    ...
    if let Ok(Some(ts_prompt)) =
      get_moon_config_safe("extension.moonshine.prompts.typescript_fix")
    {
    ...
    if let Ok(Some(tsdoc_prompt)) =
      get_moon_config_safe("extension.moonshine.prompts.tsdoc_enhancement")
    {
    ...
    if let Ok(Some(opt_prompt)) =
      get_moon_config_safe("extension.moonshine.prompts.code_optimization")
    {
    ...
    if let Ok(Some(complexity_prompt)) =
      get_moon_config_safe("extension.moonshine.prompts.complexity_reduction")
    {
    ...
    if let Ok(Some(optimized_json)) =
      get_moon_config_safe("extension.moonshine.optimized_prompts")
    {
      if let Ok(optimized_prompts) = serde_json::from_str::<
      ...
      >(&optimized_json)
      {
        ...
        if let Some(opt_complexity) =
          optimized_prompts.get("complexity_reduction")
        {
  ...
  fn validate_prompt_template(prompt: &str) -> anyhow::Result<bool> {
    ...
    if prompt.len() > 10000 {
      return Err(anyhow::anyhow!(
        "Prompt template too long: {} characters",
        prompt.len()
      ));
    }
    ...
    let security_checks =
      ["Return ONLY", "ONLY the", "JSON format", "fixed code"];

    let has_security_markers =
      security_checks.iter().any(|marker| prompt.contains(marker));
    ...
    for var in template_vars {
      if prompt.contains(&var.replace("{", "").replace("}", ""))
        && !prompt.contains(var)
      {
  ...
  pub fn optimize_prompt_for_task(
    &mut self,
    task_type: MoonTaskType,
    context: &CodeFixingContext,
  ...
  pub async fn execute_multi_step_optimization(
    ...
    if let Some(eslint_output) = &moon_task_results.eslint_output {
      let eslint_prompt = self.optimize_prompt_for_task(
        MoonTaskType::ESLintFix,
        context,
        Some(eslint_output),
      );

      let eslint_result = self
        .claude_adapter
        .execute_provider_optimized_prompt(&eslint_prompt, context)
        .await?;
    ...
    if let Some(tsc_output) = &moon_task_results.typescript_output {
      let ts_prompt = self.optimize_prompt_for_task(
        MoonTaskType::TypeScriptFix,
        context,
        Some(tsc_output),
      );
      ...
      let ts_result = self
        .claude_adapter
        .execute_provider_optimized_prompt(&ts_prompt, context)
        .await?;
    ...
    if context.file_type == "typescript" || context.file_type == "javascript" {
      let tsdoc_prompt = self.optimize_prompt_for_task(
        MoonTaskType::TSDocEnhancement,
        context,
        None,
      );

      let tsdoc_result = self
        .claude_adapter
        .execute_provider_optimized_prompt(&tsdoc_prompt, context)
        .await?;
    ...
    if context.complexity_score > 15.0 {
      let optimization_prompt = self.optimize_prompt_for_task(
        MoonTaskType::ComplexityReduction,
        context,
        None,
      );
      ...
      let optimization_result = self
        .claude_adapter
        .execute_provider_optimized_prompt(&optimization_prompt, context)
        .await?;
  ...
  pub fn parse_tool_outputs_to_context(
    ...
    let (error_count, warning_count, eslint_rules) =
      if let Some(eslint) = eslint_output {
    ...
    if !tsc_errors.is_empty() {
      previous_fixes
        .push(format!("TypeScript errors: {} issues", tsc_errors.len()));
    }
  ...
  fn parse_eslint_output(eslint_output: &str) -> (usize, usize, Vec<String>) {
    ...
    if let Ok(eslint_json) =
      serde_json::from_str::<serde_json::Value>(eslint_output)
    {
      ...
      if let Some(files) = eslint_json.as_array() {
        for file in files {
          if let Some(messages) =
            file.get("messages").and_then(|m| m.as_array())
          {
            for message in messages {
              if let Some(severity) =
                message.get("severity").and_then(|s| s.as_u64())
              {
                match severity {
    ...
    let warning_count =
      lines.iter().filter(|line| line.contains("warning")).count();
  ...
  fn calculate_code_complexity(code: &str) -> f32 {
    ...
    (lines as f32 * 0.1)
      + (functions as f32 * 2.0)
      + (conditions as f32 * 1.5)
      + (loops as f32 * 2.0)
  }
  ...
  pub fn generate_optimized_prompt(context: &CodeFixingContext) -> String {
    ...
    prompt_parts.push(format!(
      "You are analyzing {} code in a {} project.",
      context.file_type, context.project_type
    ));

    // Specific instructions based on error types
    if context.error_count > 0 {
      prompt_parts.push(format!(
        "Fix {} critical errors with highest priority.",
        context.error_count
      ));
    }
    ...
    if context.complexity_score > 20.0 {
      prompt_parts.push(
        "Focus on simplifying complex logic and improving readability."
          .to_string(),
      );
    } else if context.complexity_score < 5.0 {
      prompt_parts.push(
        "Enhance code with better error handling and documentation."
          .to_string(),
      );
    }
    ...
    match context.project_type.as_str() {
      ...
      "backend" => {
        prompt_parts.push(
          "Focus on security, performance, and proper async/await patterns."
            .to_string(),
        );
      }
      _ => {
        prompt_parts.push(
          "Apply general TypeScript/JavaScript best practices.".to_string(),
        );
      }
    }
...
class DataProcessor {
  ...
  // DSPy embedded - always available
  pub async fn generate_and_execute_fix(
    &mut self,
    context: &CodeFixingContext,
  ) -> Result<ValidatedCodeFix, Box<dyn std::error::Error>> {
    ...
    example_data.insert(
      "file_path".to_string(),
      serde_json::Value::String(context.file_path.clone()),
    );
    example_data.insert(
      "file_type".to_string(),
      serde_json::Value::String(context.file_type.clone()),
    );
    example_data.insert(
      "error_count".to_string(),
      serde_json::Value::String(context.error_count.to_string()),
    );
    example_data.insert(
      "warning_count".to_string(),
      serde_json::Value::String(context.warning_count.to_string()),
    );
    example_data.insert(
      "project_type".to_string(),
      serde_json::Value::String(context.project_type.clone()),
    );
    example_data.insert(
    ...
    example_data.insert(
      ...
      serde_json::Value::String(context.previous_fixes.join(", ")),
    );
    example_data.insert(
      "code_snippet".to_string(),
      serde_json::Value::String(context.code_snippet.clone()),
    );
    ...
    let claude_response = self
      .claude_adapter
      .execute_provider_optimized_prompt(&optimized_prompt, context)
      .await?;
  ...
  pub fn validate_claude_response(
    ...
    let parsed_response: ValidatedCodeFix = serde_json::from_str(response)
  ...
  pub async fn optimize_prompts(
      ...
      .map(|training_example| {
        ...
        data.insert(
          ...
          serde_json::Value::String(
            training_example.input_context.file_path.clone(),
          ),
        );
        data.insert(
          "file_type".to_string(),
          serde_json::Value::String(
            training_example.input_context.file_type.clone(),
          ),
        );
        data.insert(
          "error_count".to_string(),
          serde_json::Value::String(
            training_example.input_context.error_count.to_string(),
          ),
        );
        data.insert(
          "warning_count".to_string(),
          serde_json::Value::String(
            training_example.input_context.warning_count.to_string(),
          ),
        );
        data.insert(
          "project_type".to_string(),
          serde_json::Value::String(
            training_example.input_context.project_type.clone(),
          ),
        );
        data.insert(
          "complexity_score".to_string(),
          serde_json::Value::String(format!(
            "{:.1}",
            training_example.input_context.complexity_score
          )),
        );
        data.insert(
          "previous_fixes".to_string(),
          serde_json::Value::String(
            training_example.input_context.previous_fixes.join(", "),
          ),
        );
        data.insert(
          "code_snippet".to_string(),
          serde_json::Value::String(
            training_example.input_context.code_snippet.clone(),
          ),
        );
        // Output fields
        data.insert(
          "fixed_code".to_string(),
          serde_json::Value::String(training_example.expected_output.clone()),
        );
        data.insert(
          "confidence".to_string(),
          serde_json::Value::String(format!(
            "{:.2}",
            training_example.quality_score
          )),
        );
    ...
    self
      .optimizer
      .compile(&mut self.code_fixing_module, dspy_examples)
      .await?;
  ...
  pub fn get_optimization_stats(&self) -> OptimizationStats {
    ...
    OptimizationStats {
      ...
      last_optimization: self
        .code_fixing_module
        .optimization_history
        .last()
        .map(|attempt| attempt.timestamp.clone()),
    }
  }
}
...
impl MultiStepResult {
  ...
  pub fn add_step_result(&mut self, step_name: &str, claude_response: String) {
    self.add_step_result_with_timing(
      step_name,
      claude_response,
  ...
  pub fn add_step_result_with_timing(
    &mut self,
    step_name: &str,
    ...
    let success = !claude_response.to_lowercase().contains("error")
      && !claude_response.to_lowercase().contains("failed")
      && !claude_response.is_empty();
...
mod tests {
  ...
  fn test_dspy_initialization() {
    let dspy_engine =
      MoonShineDSPy::new("test-session".to_string(), "sonnet".to_string());
  ...
  fn test_claude_xml_prompt_formatting() {
    let adapter = ClaudeCLIAdapter::with_provider_detection(
  ...
  fn test_provider_optimized_prompt_generation() {
    ...
    let claude_adapter = ClaudeCLIAdapter::with_provider_detection(
      "test-session".to_string(),
      "sonnet".to_string()
    ).unwrap();
    ...
    let gemini_adapter = ClaudeCLIAdapter::with_provider_detection(
      "test-session".to_string(),
      "gemini-2.5-pro".to_string()
    ).unwrap();
    ...
    let openai_adapter = ClaudeCLIAdapter::with_provider_detection(
      "test-session".to_string(),
      "gpt4.1".to_string()
    ).unwrap();
  ...
  fn test_provider_config_serialization() {
    let config = ProviderConfig {
      provider_type: LLMProvider::Claude { version: "claude-3-5-sonnet".to_string() },
  ...
  fn test_complex_code_snippet_handling() {
    let adapters = [
      ("claude", ClaudeCLIAdapter::with_provider_detection("test".to_string(), "sonnet".to_string()).unwrap()),
      ("gemini", ClaudeCLIAdapter::with_provider_detection("test".to_string(), "gemini-2.5-pro".to_string()).unwrap()),
      ("openai", ClaudeCLIAdapter::with_provider_detection("test".to_string(), "gpt4.1".to_string()).unwrap()),
    ];
  ...
  fn test_eslint_parsing() {
    ...
    let (errors, warnings, rules) =
      MoonShineDSPy::parse_eslint_output(eslint_json);
    assert_eq!(errors, 1);
...
function processData(data) {
    ...
    let simple_complexity =
      MoonShineDSPy::calculate_code_complexity(simple_code);
    let complex_complexity =
      MoonShineDSPy::calculate_code_complexity(complex_code);
  ...
  fn test_project_type_inference() {
    assert_eq!(
      MoonShineDSPy::infer_project_type("src/components/Button.tsx"),
      "react_app"
    );
    assert_eq!(
      MoonShineDSPy::infer_project_type("lib/utils/helpers.ts"),
      "library"
    );
    assert_eq!(
      MoonShineDSPy::infer_project_type("src/api/routes.ts"),
      "backend"
    );
    assert_eq!(
      MoonShineDSPy::infer_project_type("tests/unit.test.ts"),
      "test"
    );

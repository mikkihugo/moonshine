...
impl ClaudeFixer {
  ...
  pub async fn fix_file_sync(
    ...
    let ai_response = fix_code_with_ai(
      session_id,
      file_path.to_string(),
      content.to_string(),
      language.to_string(),
    ...
    let fixed_errors =
      self.count_fixed_errors(content, &fixed_content, language)?;

    // Calculate final TSDoc coverage if content was actually fixed
    let final_tsdoc_coverage =
      if language == "typescript" || language == "javascript" {
    ...
    if language == "typescript" || language == "javascript" {
      info!("TSDoc Coverage Analysis for {}: Initial {:.1}% -> Final {:.1}% (Δ{:+.1}%)",
    ...
    let enhanced_success = ai_response.success
      && (fixed_errors > 0
        || tsdoc_improvement_significant
  ...
  fn build_ai_prompt(
    ...
    let language_specific_instructions = match language {
        "typescript" => {
            if self.config.enable_ai_tsdoc.unwrap_or(true) {
                ...
                "Fix TypeScript issues and improve type safety. Use strict types and modern TypeScript patterns."
                    .to_string()
            }
        },
    ...
    let prompt = format!(
      ...
      {
        format!("Fix TypeScript issues, improve type safety, and add comprehensive TSDoc comments. \
                    Target TSDoc coverage: {:.1}%. Use strict types and modern TypeScript patterns.",
  ...
  fn count_fixed_errors(
    ...
    changes += (original_lines.len() as i32 - fixed_lines.len() as i32)
    ...
    if language == "typescript" || language == "javascript" {
      ...
      changes +=
        (fixed_tsdoc_count.saturating_sub(original_tsdoc_count)) as usize;
    }
...
fn calculate_tsdoc_coverage(content: &str) -> f64 {
  let function_regex = match regex::Regex::new(
  ...
  let method_regex = match regex::Regex::new(
    r"(?m)^\s*(?:public\s+|private\s+|protected\s+)?(?:async\s+)?(\w+)\s*\(",
  ) {
    Ok(regex) => regex,
    Err(_) => return 0.0, // Graceful fallback if regex compilation fails
  };
...
"#;
  ...
  fn test_serialization() {
    ...
    // Test serialization
    let serialized =
      serde_json::to_string(&result).expect("Should serialize to JSON");
    ...
    let deserialized: AiCodeFixResult =

...
mod tests {
  ...
  fn test_example_creation() {
    ...
    let example =
      Example::new(data.clone(), input_keys.clone(), output_keys.clone());
  ...
  fn test_prediction_creation() {
    let data = HashMap::from([
      ("result".to_string(), json!("predicted output")),
  ...
  fn test_prediction_serialization() {
    let data = HashMap::from([
      ("output".to_string(), json!("generated text")),
      ("score".to_string(), json!(0.87)),
    ]);
    ...
    assert_eq!(
      deserialized.lm_usage.total_tokens,
      prediction.lm_usage.total_tokens
    );
  }

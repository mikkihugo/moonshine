...
impl AIProviderConfig {
  ...
  pub fn claude() -> Self {
    ...
    let api_key_env = if uses_oauth {
      None
    } else {
      config.claude_api_key_env.clone()
    };
  ...
  pub fn google() -> Self {
    ...
    let (code_analysis, code_generation, complex_reasoning, speed) =
      match model.as_str() {
    ...
    let api_key_env = if uses_oauth {
      ...
      config.gemini_api_key_env.clone()
    };
  ...
  pub fn openai() -> Self {
    ...
    let api_key_env = if uses_oauth {
      ...
      config.codex_api_key_env.clone()
    };
    ...
    Self {
      ...
      capabilities: ProviderCapabilities {
        ...
        complex_reasoning: config
          .codex_complex_reasoning_rating
          .unwrap_or(0.85),
...
impl AIRouter {
  ...
  pub fn new() -> Self {
    Self {
      providers: vec![
        AIProviderConfig::claude(),
        AIProviderConfig::google(),
  ...
  pub fn select_provider(
    ...
    if let Some((score, provider, reason)) = ranked.pop() {
      info!(
        "AI Provider Selected: {} (Score: {:.2}) - {}",
        provider.name, score, reason
      );
      Ok((provider, format!("Score: {:.2} - {}", score, reason)))
    } else {
      let dummy_config = AIProviderConfig::claude();
      Err(Error::ai_execution(
        &dummy_config,
        "No AI providers available - check API key configuration",
      ))
    }
  }
  ...
  fn rank_providers(
    &self,
    request: &AIRequest,
  ) -> Vec<(f32, &AIProviderConfig, String)> {
    ...
    let mut scored_providers: Vec<(f32, &AIProviderConfig, String)> = self
      ...
      .filter_map(|provider| {
        ...
        let (mut score, mut reason) =
          self.score_provider(provider, &requirements);
        ...
        {
          let preference_boost =
            1.0 + (request.preferred_providers.len() - position) as f32 * 0.1;
  ...
  fn score_provider(
    ...
    if requirements.needs_code_analysis {
      ...
      reasons.push(format!(
        "code analysis: {:.2}",
        provider.capabilities.code_analysis
      ));
    }
    ...
    if requirements.needs_code_generation {
      ...
      reasons.push(format!(
        "code generation: {:.2}",
        provider.capabilities.code_generation
      ));
    }
    ...
    if requirements.needs_complex_reasoning {
      ...
      reasons.push(format!(
        "reasoning: {:.2}",
        provider.capabilities.complex_reasoning
      ));
    }
    ...
    if !requirements.needs_code_analysis
      && !requirements.needs_code_generation
      && !requirements.needs_complex_reasoning
    {
      score = (provider.capabilities.code_analysis
        + provider.capabilities.code_generation
        + provider.capabilities.complex_reasoning)
        / 3.0;
  ...
  fn execute_with_provider(
    ...
    let output = execute_command(command_input).map_err(|e| {
      Error::ai_execution(provider, format!("AI CLI execution failed: {}", e))
    })?;
  ...
  fn build_provider_args(
    ...
    match provider.name.as_str() {
      "claude" => {
        ...
        match &request.context {
          AIContext::CodeFix { .. } => {
            ...
            if let Some(file_path) = &request.file_path {
              args
                .extend_from_slice(&["--file".to_string(), file_path.clone()]);
            }
          }
        ...
        // Add prompt
        args
          .extend_from_slice(&["--prompt".to_string(), request.prompt.clone()]);
      }
  ...
  fn get_working_directory(&self, request: &AIRequest) -> Option<String> {
    ...
    match &request.context {
      AIContext::DSPyOptimization { .. } => {
        Some(format!("/tmp/moon-shine-dspy/{}", request.session_id))
      }
      AIContext::CodeFix { .. } => {
        Some(format!("/tmp/moon-shine-fixes/{}", request.session_id))
      }
      AIContext::CodeGeneration { .. } => {
        Some(format!("/tmp/moon-shine-generation/{}", request.session_id))
      }
      AIContext::CodeAnalysis { .. } => {
        Some(format!("/tmp/moon-shine-analysis/{}", request.session_id))
      }
      AIContext::General => {
        Some(format!("/tmp/moon-shine-general/{}", request.session_id))
      }
    }
  }
...
pub async fn optimize_with_dspy(
  session_id: String,
  signature: String,
  messages: Vec<String>,
  prompt: String,
  ...
  let request = AIRequest {
    ...
    session_id,
    file_path: None,
    context: AIContext::DSPyOptimization {
      signature,
      messages,
    },
    preferred_providers: Vec::new(),
...
pub async fn execute_ai_prompt(
  session_id: String,
  prompt: String,
) -> Result<AIResponse> {
  let router = get_ai_router();
...
pub fn batch_process_files(
  ...
  for batch in files.chunks(batch_size) {
    ...
    info!(
      "Processed batch of {} files, total suggestions: {}",
      batch.len(),
      all_suggestions.len()
    );
  }
...
mod tests {
  ...
  fn test_ai_context_variants() {
    ...
    match dspy_optimization {
      ...
      _ => panic!(
        "Expected DSPyOptimization context but got different AIContext variant"
      ),
    }
    ...
    match code_analysis {
      ...
      _ => panic!(
        "Expected CodeAnalysis context but got different AIContext variant"
      ),
    }
    ...
    match code_generation {
      ...
      _ => panic!(
        "Expected CodeGeneration context but got different AIContext variant"
      ),
    }
  ...
  fn test_batch_process_files() {
    ...
    let processor = |batch: &[String]| -> std::result::Result<
      Vec<LintIssue>,
      Box<dyn std::error::Error>,
    > {
  ...
  fn test_batch_process_files_processor_error() {
    ...
    let processor = |_batch: &[String]| -> std::result::Result<
      Vec<LintIssue>,
      Box<dyn std::error::Error>,
  ...
  fn test_serialization_and_deserialization() {
    ...
    assert!(
      (deserialized.code_analysis - capabilities.code_analysis).abs()
        < f32::EPSILON
    );
    assert_eq!(deserialized.context_length, capabilities.context_length);
    assert_eq!(
      deserialized.supports_sessions,
      capabilities.supports_sessions
    );
  }

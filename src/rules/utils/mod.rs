...
/// Extract 1-based line and column from OXC span with source text analysis
/// Production implementation supporting various line ending formats
pub fn span_to_line_col(source_text: &str, span: Span) -> (u32, u32) {
    span_to_line_col_with_cache(source_text, span, None)
}

/// Extract line and column with optional line offset cache for performance
/// Optimized for repeated calls on the same source text
...
/// Calculate line number by counting line breaks before the offset
...
/// Calculate column number from the start of the current line
...
/// Find the byte offset of the start of the line containing the given offset
...
/// Find line number using pre-computed line offsets (performance optimization)
...
/// Estimate line/column when offset is beyond source text end
...
/// Generate line offset cache for efficient repeated span conversions
/// Call this once per source file when processing many spans
...
/// Compatibility wrapper for existing code expecting Program parameter
...
/// Convert string to PascalCase with Unicode safety
...
/// Generate boolean name suggestions for a given name
...
/// Check if a name starts with a boolean prefix

...
impl<'a> ComplexityAnalyzer<'a> {
  ...
  pub fn finalize(self) -> ComplexityMetrics {
    ...
    ComplexityMetrics {
      ...
      instability: if self.import_count + 0 > 0 { self.import_count as f64 / (self.import_count + 0) as f64 } else { 0.0 },
  ...
  fn calculate_dependency_complexity(&self) -> u32 {
    ...
    let total_complexity = import_complexity
      + dynamic_import_penalty
      + namespace_import_penalty
      + export_complexity
      + circular_risk;

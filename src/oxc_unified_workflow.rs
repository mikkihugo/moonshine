//! # OXC Unified Workflow - Replace External Tools with Oxide Stack
//!
//! This module implements a unified analysis workflow using the complete OXC (Oxidation Compiler)
...
//! - Unified error reporting and source map generation
//! - Memory-efficient arena-based allocation for large codebases
//!
//! ## Replaced Tools
//! - `tsc` → `oxc_parser` + `oxc_semantic` (type checking)
//! - `eslint` → `oxc_linter` + `oxc_transformer` (linting + fixes)
//! - `prettier` → `oxc_codegen` (formatting)
//! - `complexity-analyzer` → Custom OXC-based metrics
//! - `tsdoc-analyzer` → OXC trivia processing
//! - Import organization → `oxc_resolver` + transformations
//!
...
/// Unified OXC-based analysis workflow result
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct OxcWorkflowResult {
...
pub struct TypeAnalysisResult {
    pub success: bool,
    pub syntax_errors: Vec<OxcDiagnostic>,
    pub type_errors: Vec<OxcDiagnostic>,
    pub semantic_errors: Vec<OxcDiagnostic>,
...
pub struct LintAnalysisResult {
    ...
    pub remaining_issues: Vec<OxcDiagnostic>,
...
/// OXC-based diagnostic with precise location information
#[derive(Debug, Clone, Serialize, Deserialize)]
...
/// Main OXC unified workflow coordinator
pub struct OxcUnifiedWorkflow {
    config: MoonShineConfig,
    allocator: Arc<Allocator>,
    diagnostic_service: DiagnosticService,
}

impl OxcUnifiedWorkflow {
    /// Create new OXC unified workflow
    ...
    /// Execute the complete unified OXC workflow
    /// Replaces the entire 11-phase external tool workflow with single-pass processing
    pub async fn execute_unified_analysis(
        &mut self,
        source_code: &str,
        file_path: &str,
    ) -> Result<OxcWorkflowResult, Error> {
        ...
        let parser_result = Parser::new(&self.allocator, source_code, source_type)
            .with_options(parser_options)
            .parse();

        if !parser_result.errors.is_empty() {
            return Ok(OxcWorkflowResult {
                success: false,
                ...
                type_analysis: TypeAnalysisResult {
                    ...
                    syntax_errors: parser_result
                        .errors
                        .into_iter()
                        .map(|e| self.convert_parser_error(e))
                        .collect(),
                    type_errors: vec![],
                    semantic_errors: vec![],
                    symbol_count: 0,
    ...
    fn perform_type_analysis(
        ...
        program: &Program,
        source_code: &str,
    ) -> Result<TypeAnalysisResult, Error> {
    ...
    fn perform_lint_analysis(
        &self,
        _program: &Program,
        _source_code: &str,
    ) -> Result<LintAnalysisResult, Error> {
    ...
    fn perform_documentation_analysis(
        &self,
        _program: &Program,
    ) -> Result<DocumentationAnalysisResult, Error> {
    ...
    fn perform_code_generation(
        &self,
        program: &Program,
        original_source: &str,
    ) -> Result<FormatAnalysisResult, Error> {
    ...
    // Helper methods
    /// Converts an OXC parser error to an OxcDiagnostic, extracting all available metadata.
    /// Logs the error and ensures all fields (rule_name, message, line, column, severity) are set for downstream consumers.
    fn convert_parser_error(&self, error: oxc_diagnostics::Error) -> OxcDiagnostic {
        ...
        eprintln!(
            "[OXC Parser Error] rule=parser message=\"{}\" line={} column={} span=({}-{})",
            error.to_string(),
            line,
            column,
            start,
    ...
    fn categorize_semantic_errors(
        &self,
        _errors: Vec<oxc_diagnostics::Error>,
    ) -> (Vec<OxcDiagnostic>, Vec<OxcDiagnostic>, Vec<OxcDiagnostic>) {
    ...
    fn collect_all_diagnostics(&self) -> Vec<OxcDiagnostic> {

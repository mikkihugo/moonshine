...
/// @complexity low
/// @since 1.0.0
pub fn load_prompt_from_storage(rule_type: &str, config: Option<&MoonShineConfig>) -> String {
...
mod tests {
    ...
    fn test_training_file_structure() {
        ...
        let examples = training.get("training_examples").unwrap().as_array().unwrap();
        assert!(examples.is_empty()); // Initially empty

        // Check optimization_history array
        let history = training.get("optimization_history").unwrap().as_array().unwrap();

use crate::dspy::{
  get_lm, Evaluator, Example, Module, Optimizable, Optimizer, Predict,
  Predictor, LM,
};
use crate::signature;
use anyhow::Result;
...
static BASIC_GENERATOR: LazyLock<Predict> = LazyLock::new(|| {
  Predict::new(BasicGenerateInstruction::new())
});

static REFINEMENT_GENERATOR: LazyLock<Predict> = LazyLock::new(|| {
  Predict::new(GenerateInstructionGivenAttempts::new())
});
...
impl Optimizer for COPRO {
  ...
  async fn compile<M: Module + Optimizable + Evaluator>(
    ...
    let mut all_candidates: HashMap<String, Vec<(String, String)>> =
      HashMap::new();
    let mut latest_candidates: HashMap<String, Vec<(String, String)>> =
      HashMap::new();
    let mut evaluated_candidates: HashMap<
      String,
      ...
      if self.breadth > 1 {
        let candidate_futures: Vec<_> = (0..self.breadth - 1)
          .map(|_| {
            ...
            async move {
              let prediction =
                if let Some(mut prompt_model) = prompt_model {
    ...
    for d in 0..self.depth {
      ...
      for (p_i, (predictor_name, _, _)) in predictor_info.iter().enumerate() {
        ...
        for (c_i, (instruction, prefix)) in
          candidates_to_eval.iter().enumerate()
        {
        ...
        {
          {
            ...
            if let Some(predictor) = module_predictors.get_mut(predictor_name) {
              predictor
                .update_signature_instruction(best.instruction.clone())?;
            }
          }
        ...
        if self.track_stats && !latest_scores.is_empty() {
          let avg =
            latest_scores.iter().sum::<f32>() / latest_scores.len() as f32;
          if let Some(latest_results) =
            stats.results_latest.get_mut(predictor_name)
          {
          ...
          if !best_scores.is_empty() {
            let best_avg =
              best_scores.iter().sum::<f32>() / best_scores.len() as f32;
      ...
      for (predictor_name, _, _) in &predictor_info {
        ...
        for (i, candidate) in
          best_candidates.iter().take(num_examples).enumerate()
        {
        ...
        if let Ok(prediction) = prediction {
          ...
          let instructions = Self::normalize_lm_response(
            &prediction.data,
            "proposed_instruction",
          );

          let prefixes = Self::normalize_lm_response(
            &prediction.data,
    ...
    if let Some((best_predictor, best_candidate)) = best_overall.clone() {
      ...
      if best_candidate.score > individual_avg_score * GLOBAL_OPTIMIZATION_THRESHOLD_FACTOR {
        let mut module_predictors = module.parameters();
        if let Some(target_predictor) =
          module_predictors.get_mut(&best_predictor)
        {
          target_predictor
            .update_signature_instruction(best_candidate.instruction.clone())?;
          if !best_candidate.prefix.is_empty() {
            target_predictor
              .update_signature_prefix(best_candidate.prefix.clone())?;
          }
          applied_global = true;
        }
      }
    ...
    if !applied_global {
      ...
      for (predictor_name, predictor) in module_predictors {
        if let Some(best) =
          evaluated_candidates.get(&predictor_name).and_then(|m| {
            m.values()
              .filter(|c| c.score.is_finite())
              .max_by(|a, b| a.score.total_cmp(&b.score))
          })
